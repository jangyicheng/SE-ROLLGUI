...
class AgenticPipeline(BasePipeline):
        ...
        for global_step in range(self.pipeline_config.max_steps):
            ...
            with Timer(name="pipeline_step_total", logger=None) as step_timer:
                with tps_timer:
                    ...
                    if self.pipeline_config.async_generation_ratio > 0:
                        self.actor_infer.start_server(
                            data=DataProto(meta_info={"global_step": global_step, "is_offload_states": False})
                        )
                    else:
                        self.actor_infer.start_server(
                            data=DataProto(meta_info={"global_step": global_step, "is_offload_states": True})
                        )
                    ...
                    with Timer(name="train_timer", logger=None) as train_timer:
                        if self.pipeline_config.adv_estimator == "gae":
                            critic_train_metrics_refs: List[ray.ObjectRef] = self.critic.train_step(
                                batch, blocking=False
                            )
                ...
                with Timer(name="log", logger=None) as log_timer:
                    if (
                        self.pipeline_config.logging_steps > 0
                        and global_step % self.pipeline_config.logging_steps == 0
                    ):
                        ...
                        for group_name, group_batch in batch_grouped.items():
                            ...
                            non_prompt_mask = (
                                torch.logical_not(group_batch.batch["prompt_mask"])
                                * group_batch.batch["attention_mask"]
                            )
    ...
    def adjust_batch(self, data: DataProto, mode="copy") -> DataProto:
        ...
        if self.pipeline_config.adv_estimator == "gae":
            critic_train_bsz = (
                self.pipeline_config.critic.training_args.per_device_train_batch_size
                * self.pipeline_config.critic.training_args.gradient_accumulation_steps
                * self.critic.dp_size
            )
            critic_infer_bsz = self.pipeline_config.critic.infer_batch_size * self.critic.dp_size

        size_divide = np.lcm.reduce(
            np.array([actor_train_train_bsz, actor_train_infer_bsz, ref_infer_bsz, critic_infer_bsz, critic_train_bsz])
        ).item()
...
def compute_data_metrics(batch):
    ...
    metrics = {
        ...
        # # traj_rollout_time
        "env/traj_rollout_time/mean": (
            torch.mean(traj_rollout_times).detach().item() if traj_rollout_times.numel() > 0 else 0.0
        ),
        "env/traj_rollout_time/max": (
            torch.max(traj_rollout_times).detach().item() if traj_rollout_times.numel() > 0 else 0.0
        ),
        "env/traj_rollout_time/min": (
            torch.min(traj_rollout_times).detach().item() if traj_rollout_times.numel() > 0 else 0.0
        ),
        # traj_env_times
        ...
        metrics.update(
            {
                ...
                "critic/values/max": (
                    torch.max(values[response_mask]).detach().item() if response_mask.sum() > 0 else 0.0
                ),
                "critic/values/min": (
                    torch.min(values[response_mask]).detach().item() if response_mask.sum() > 0 else 0.0
                ),
            }
        )
