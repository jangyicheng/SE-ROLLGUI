...
class AndroidStepEnvManager(GuiTrajEnvManager):
    def __init__(
        ...
        **kwargs,
        ...
        self.keep_last_k = self.env_config.get("keep_last_k", None)  # 兼容保留，不再驱动MAI对齐路径
        self.history_n = self.env_config.get("history_n", self.keep_last_k or 3)
    ...
    def format_messages(self, rollout_cache: RolloutCache) -> Tuple[DataProto, List[Dict]]:
        def encode_observation_as_png(observation: np.ndarray) -> Tuple[str, PIL.Image.Image]:
            image_pil = PIL.Image.fromarray(observation, mode="RGB")
            image_buffer = BytesIO()
            image_pil.save(image_buffer, format="PNG")
            encoded_image = base64.b64encode(image_buffer.getvalue()).decode("utf-8")
            return encoded_image, image_pil

        def build_image_message(encoded_image: str) -> Dict:
            return {
                "role": "user",
                "content": [
                    {
        ...
        instruction_text = self.env.task["goal"]
        ...
        messages = [
            {
                "role": "system",
                "content": [{"type": "text", "text": self.agent_system_template}],
            },
            build_text_message("user", instruction_text),
        ]

        images: List[PIL.Image.Image] = []

        history_steps = [
            step for step in rollout_cache.history[:-1]
            if "llm_response" in step and "observation" in step
        ]
        start_image_idx = max(0, len(history_steps) - (self.history_n - 1))

        for history_idx, step in enumerate(history_steps):
            should_include_image = history_idx >= start_image_idx
            if should_include_image:
                encoded_image, image_pil = encode_observation_as_png(step["observation"])
                messages.append(build_image_message(encoded_image))
                images.append(image_pil)

            history_response = step["llm_response"]
            if self.tokenizer.eos_token and history_response.endswith(self.tokenizer.eos_token):
                history_response = history_response[: -len(self.tokenizer.eos_token)]
            messages.append(build_text_message("assistant", history_response))

            ask_user_response = step.get("ask_user_response")
            if ask_user_response:
                messages.append(build_text_message("user", ask_user_response))

            mcp_response = step.get("mcp_response")
            if mcp_response:
                messages.append(build_text_message("user", mcp_response))

        current_cache = rollout_cache.history[-1]
        current_observation = current_cache["observation"]
        current_encoded_image, current_image_pil = encode_observation_as_png(current_observation)
        messages.append(build_image_message(current_encoded_image))
        images.append(current_image_pil)
        ...
        features = [
            {
                ...
                self.collator.image_flag_key: len(images) > 0,
            }
        ]
