...
class AndroidStepEnvManager(GuiTrajEnvManager):
    def __init__(
        ...
        **kwargs,
        ...
        self.keep_last_k = self.env_config.get("keep_last_k", None)  # 仅保留最近的k轮对话历史
    ...
    def make_decision(self, rollout_cache: RolloutCache):
        ...
        content["memory"] = extract_memory(response_ids)
    ...
    def format_messages(self, rollout_cache: RolloutCache) -> Tuple[DataProto, List[Dict]]:
        ...
        if len(rollout_cache.history) > 0:
            last_content = rollout_cache.history[-1]
            # 优先使用已经存好的 memory（推荐方式）
            if "memory" in last_content and last_content["memory"]:
                latest_memory = last_content["memory"]
            # 兼容旧数据或首次过渡期，从 llm_response 提取
            elif "llm_response" in last_content:
                latest_memory = extract_memory(last_content["llm_response"])

        # ==================== 构建 user prompt（替换原来的 previous_action 逻辑）================
        user_text = f"""Previous compressed memory (must be updated in your new <memory> this turn):
        {latest_memory if latest_memory else 'None'}

        The user query: {self.env.task["goal"]}
        Task progress and all history is already summarized in the memory above. Use it and compress new information this turn."""

        images = []
        # 只使用当前最新的 observation（和原来逻辑一致）
        current_content = rollout_cache.history[-1] if rollout_cache.history else {}
        obs = current_content.get("observation") if "observation" in current_content else None
        if obs is not None:
            images.append([PIL.Image.fromarray(obs, mode="RGB")])

        messages.append(
            build_user_content(
                user_text,
                base64.b64encode(current_content["observation"]).decode("utf-8") if "observation" in current_content else "",
        ...
        # 后面的 collator 和 position_ids 逻辑保持完全不变
...
class AndroidEnvGroupFilter:
    ...
    def filter(self, group_id: int, episode_id: int, group: list[DataProto]):
        ...
        logger.info("filter is deprecated, no filter is applied")
        return False
