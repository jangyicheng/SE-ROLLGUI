...
from vllm.engine.arg_utils import ConvertOption, EngineArgs, HfOverrides, PoolerConfig, RunnerOption
...
class Llm0110(LLM):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.llm_engine = LLMEngine0110.from_engine_args(engine_args=engine_args, usage_context=UsageContext.LLM_CLASS)
    ...
    def update_parameter(self, parameter_name, weight, ranks_in_worker, is_lora):
        if envs.VLLM_USE_V1:
            weight_dict = {"dtype": weight.dtype, "weight": weight.cpu().tolist()}
...
def get_mm_mapping(self) -> "MultiModelKeys":
    """Get the multi-model keys mapping for Qwen3-VL model."""
...
Qwen3VLForConditionalGeneration.get_mm_mapping = get_mm_mapping
